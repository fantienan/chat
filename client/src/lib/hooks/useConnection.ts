...
const DEFAULT_REQUEST_TIMEOUT_MSEC = parseInt(params.get('timeout') ?? '') || 10000;
...
export function useConnection({
  ...
  const makeRequest = async <T extends z.ZodType>(request: ClientRequest, schema: T, options?: RequestOptions): Promise<z.output<T>> => {
    if (!mcpClient) {
